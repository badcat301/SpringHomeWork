...
//        Student student = (Student) beanFactory.getBean("studens");
        ...
        findStudentByGroup(students);

        System.out.printf("Task#2"+"\n");
        System.out.printf("===================="+"\n");

        findStudentByPoint(students);

        System.out.printf("Task#3"+"\n");
        System.out.printf("===================="+"\n");

        convertStudenToProfessional(students);

        //.map(Student::getStudentName).forEach(System.out::println); //вывести весь лист

    }

    private static void convertStudenToProfessional(ArrayList<Student> students) {
        students.stream()
    ...
    private static void findStudentByPoint(ArrayList<Student> students) {
        students.stream()
                .filter(student -> student.getGroupNumber()==2&& student.getPointOfstudy()>80)
        .forEach(System.out::println); //task #2
    }

    private static void findStudentByGroup(ArrayList<Student> students) {
        students.stream()
